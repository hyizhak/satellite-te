...
def Layer(index, reduceF):
    if index < 1584/reduceF:
        return 0 
    elif index < 3168/reduceF:
        return 1
    elif index < 3516/reduceF:
        return 2
    else:
        return 3
...
def SPOnGrid(src, des, G_interShell, ISL_interShell, mode:ISM, pathN, reduceF):
    Offset = [0, round(1584/reduceF), round(3168/reduceF)]
    OrbitN = [round(72/reduceF), round(72/reduceF)]
    SatN   = [22, 22]
    Lsrc = Layer(src, reduceF)
    Ldes = Layer(des, reduceF)
    # print(['Layers:',Lsrc,Ldes,src,des])
    if Lsrc == Ldes: # Intra-layer paths
        OffSD = Offset[Lsrc]
        # print([src, des, OffSD, OrbitN[Lsrc],SatN[Lsrc], pathN])
        # print(LGP.k_shortest_path_intra_shell(src, 
                                            # des, 
                                            # OffSD, 
                                            # OrbitN[Lsrc], 
                                            # SatN[Lsrc], 
        ...
        if mode == ISM.GRD_STATION:
            ...
            if  Lsrc < Ldes:
                ...
                PathIntra = LGP.k_shortest_path_intra_shell(src, 
                                                          ...
                                                          pathN)
                for p in PathIntra:
                    PathComplete.append(p+[int(GS + Offset[2])]+PathIntra_High[0])
                return PathComplete
            else:
                ...
                PathIntra = LGP.k_shortest_path_intra_shell(Inter + OffD, 
                                                          ...
                                                          pathN)
                for p in PathIntra:
                    PathComplete.append(PathIntra_High[0] + [int(GS + Offset[2])] + p)
                return PathComplete
        else:
            if Lsrc > Ldes:
                N = src - Offset[Lsrc]
