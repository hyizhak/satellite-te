...
class AssetManager():
    ...
    def _data_topo_dir(cls, problem_path, topo_idx, create=False):
        path = os.path.join(cls._data_dir(problem_path), f"{cls.TOPO_PREFIX}{topo_idx}")
        if create:
    ...
    @classmethod
    def _graph_path(cls, problem_path, topo_idx, create_path=False):
    ...
    def load_graph(cls, problem_path, topo_idx) -> nx.Graph:
        return nx.read_gpickle(cls._graph_path(problem_path, topo_idx))
    
    @classmethod
    def save_graph_(cls, problem_path, topo_idx, G: nx.Graph):
        nx.write_gpickle(G, cls._graph_path(problem_path, topo_idx, True))
    ...
    @classmethod
    def pathdict_path(cls, prefix, topo_idx, pathform_id, create_path=False):
        path = os.path.join(prefix, f'topo_{topo_idx}', f'{pathform_id}.pkl')
        if create_path:
            os.makedirs(os.path.dirname(path), exist_ok=True)
        return path
        
    @classmethod
    def load_pathdict(cls, prefix, topo_idx, path_id):
        with open(cls.pathdict_path(prefix, topo_idx, path_id, False), 'rb') as f:
            return pickle.load(f)
    ...
    @classmethod
    def model_dir(cls, work_dir, topo_idx, create_dir=False):
