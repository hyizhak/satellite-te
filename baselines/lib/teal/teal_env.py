...
class TealEnv(object):
    ...
    def __init__(
            self, obj, problem_path, topo_idx, 
            path_prefix, path_id, path_num,
            rho,
            num_failure, device,
            work_dir,
    ...
    def reset(self, mode='test'):
        ...
        if mode == 'train' or mode == 'validate':
            self.tm_dataset = self.train_dataset
        elif mode == 'test':
            self.tm_dataset = self.test_dataset
        else:
            raise ValueError()
        ...
        if mode == 'train':
            if self.train_all:
                self.idx_start, self.idx_stop = 0, int(len(self.tm_dataset) * (1 - self.val_ratio))
            else:
                self.idx_start, self.idx_stop = 0, self.train_size
        elif mode == 'val':
            if self.train_all:
                self.idx_start, self.idx_stop = int(len(self.tm_dataset) * (1 - self.val_ratio)), len(self.tm_dataset)
            else:
                self.idx_start, self.idx_stop = self.train_size, self.train_size + self.val_size
        else:
            if self.test_all:
                self.idx_start, self.idx_stop = 0, len(self.tm_dataset)
            else:
    ...
    def _read_obs(self):
        ...
        if self.num_failure > 0 and self.mode == 'test':
    ...
    def step(self, raw_action, sample_num=0, admm_step_num=0):
        """Return the reward of current action.

        Args:
            raw_action: raw action from actor
            sample_num: number of samples for reward during training
            admm_step_num: number of ADMM steps during testing
        """

        info = {}
        if self.mode == 'train':
            reward = self.take_action(raw_action, sample_num)
        else:
            start_time = time.time()
            action = self.transform_raw_action(raw_action)
            if self.obj == 'total_flow':
                # total flow require no constraint violation
                action = self.ADMM.tune_action(self.obs, action, admm_step_num)
    ...
    def read_graph(self) -> networkx.Graph:
        ...
        return AssetManager.load_graph(self.problem_path, self.topo_idx)

    def get_path(self):
        """Return path dictionary."""
        
        self.path_fname = path_fname = AssetManager.pathdict_path(self.path_prefix, self.topo_idx, self.path_id)
        
        print(f"Loading paths from pickle file {path_fname}")
        with open(path_fname, 'rb') as f:
            path_dict = pickle.load(f)
            print("path_dict size:", len(path_dict))
            return path_dict

    def get_regular_path(self):
