...
from lib.lp.graph_utils import compute_residual_problem
...
class POP(PathFormulation):
    ...
    def __init__(
        ...
        **addl_kwargs,
        ...
        self._num_subproblems = int(num_subproblems)
    ...
    def solve(self, problem):
        ...
        self._algos = [
            ...
            for _ in range(int(self._num_subproblems))
        ]
        self._paths_dict = self.get_paths()
    ...
    def get_paths(self):
        if not hasattr(self, "_paths_dict"):
            self._paths_dict = POP.read_paths_from_disk_or_compute(
                self.problem_path, self.topo_idx, self.path_num, self.edge_disjoint, self.dist_metric
            )
        return self._paths_dict
    @staticmethod
    def read_paths_from_disk_or_compute(problem_path, topo_idx, path_num, edge_disjoint, dist_metric):
        paths_fname = POP.paths_full_fname(
            problem_path, topo_idx, path_num, edge_disjoint, dist_metric
        )
        try:
            ...
            print("Creating paths {}".format(paths_fname))
            paths_dict = POP.compute_paths(
                problem_path, topo_idx, path_num, edge_disjoint, dist_metric
            )
