...
from .load_starlink import construct_from_edge


PATHS_DIR = os.path.join(TOPOLOGIES_DIR, "paths", "path-form")


class TopFormulation(AbstractFormulation):
    @classmethod
    def new_total_flow(
        cls, num_paths, top_percentage, edge_disjoint=True, dist_metric="inv-cap", out=None
    ):
        return cls(
    ...
    @classmethod
    def get_pf_for_obj(cls, objective, num_paths, top_percentage, **kargs):
        if objective == Objective.TOTAL_FLOW:
            return cls.new_total_flow(num_paths, top_percentage, **kargs)
        elif objective == Objective.MAX_CONCURRENT_FLOW:
            return cls.new_max_concurrent_flow(num_paths, top_percentage, **kargs)
        elif objective == Objective.MIN_MAX_LINK_UTIL:
            return cls.new_min_max_link_util(num_paths, top_percentage, **kargs)
        elif objective == Objective.COMPUTE_DEMAND_SCALE_FACTOR:
            return cls.compute_demand_scale_factor(num_paths, top_percentage, **kargs)
        else:
    ...
    @staticmethod
    def paths_full_fname(problem_path, topo_idx, num_path, edge_disjoint, dist_metric):
    ...
    def get_paths(self):
        if not hasattr(self, "_paths_dict"):
            self._paths_dict = TopFormulation.compute_paths(
                self.data, self.path_num, self.edge_disjoint, self.mode
            )
            ...
            if self._objective == Objective.TOTAL_FLOW:
                ...
                for k, (s_k, t_k, d_k) in self.commodity_list:
                    ...
                    capacity = min([d_k] + [capacity_dict[(u, v)] for u, v
                        in zip(
                            self._paths_dict[f'{s_k}, {t_k}'][0][:-1],
                            self._paths_dict[f'{s_k}, {t_k}'][0][1:])])
    ...
    def fib_entries(cls, problem, num_paths, top_percentage, edge_disjoint, dist_metric):
        assert problem.is_traffic_matrix_full
        pf = cls.new_total_flow(
            num_paths=num_paths, top_percentage=top_percentage, edge_disjoint=edge_disjoint, dist_metric=dist_metric
        )
