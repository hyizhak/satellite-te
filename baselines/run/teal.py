...
from lib.data.starlink import StarlinkTopoTMDataset
...
def benchmark(args):
    ...
    train_id_epoch_template = _teal_train_id_epoch_template(
        train_sz=trainval_tm_per_topo, val_ratio=val_ratio,
        lr=lr, batch_sz=batch_size,
        layer_num=layer_num,
        coma_sample=sample_num,
        rho=rho, admm_step=admm_step_num,
        path_id = path_id)
    
    train_id = train_id_epoch_template.format(epoch_num)
    
    for topo_idx in range(topo_num):
        
        train_dataset = StarlinkTopoTMDataset(problem_path, topo_idx, 'train')
        test_dataset = StarlinkTopoTMDataset(problem_path, topo_idx, 'test')
    ...
    if train:
        ...
        for idx, teal in enumerate(teal_instances):
            ...
            teal.train(
                ...
                save_per_epoch=True,
                epoch_model_path_template=os.path.join(AssetManager.model_dir(work_dir, idx, create_dir=True), f'{train_id_epoch_template}.pt')
            )
...
def teal_train_id(
        ...
        lr, batch_sz,  # training hyperparams
        layer_num,              # FlowGNN hyperparams
        coma_sample,            # RL hyperparams
        rho, admm_step, epoches):        # ADMM hyperparams
    return _teal_train_id_epoch_template(
        topo_num, train_sz, val_ratio,
        lr, batch_sz,
        layer_num,
        coma_sample,
        rho, admm_step).format(epoches)
    
def _teal_train_id_epoch_template(
    train_sz, val_ratio,    # training data set
    lr, batch_sz,  # training hyperparams
    layer_num,              # FlowGNN hyperparams
    coma_sample,            # RL hyperparams
    rho, admm_step, 
    ...
    return f'teal_tsz-{train_sz}_vr-{val_ratio}_' + \
    f'lr-{lr}_ep-{"{}"}_bsz-{batch_sz}_' +\
    f'layer-{layer_num}_comasample-{coma_sample}_' +\
    f'rho-{rho}_admmstep-{admm_step}_pathid-{path_id}'
...
if __name__ == '__main__':
    ...
    # path form arguments
    parser.add_argument('--path-prefix', type=str, required=True)
    parser.add_argument('--path-id', type=str, required=True)
    parser.add_argument('--path-num', type=int, default=ARG_PATH_NUM)
    
    # output parameters
    parser.add_argument("--output-dir", type=str, default=ARG_OUTPUT_DIR)
    parser.add_argument("--output-name", type=str, default=ARG_OUTPUT_NAME)

    parser.add_argument('--devid', type=int, default=0, help='GPU device id')
    
    # env hyper-parameters
    parser.add_argument('--train-tm-per-topo', type=int, default=ARG_TRAIN_TM_PER_TOPO)
    ...
    parser.add_argument('--epochs', type=int, default=ARG_EPOCHS, help='number of training epochs')
    ...
    if args.output_name is None:
        args.output_name = Path(args.problem_path).name

    args.work_dir = Path(args.output_dir) / args.output_name
