...
class IridiumAdapter():
    ...
    @classmethod
    def _link(cls, source:int, target:int, capacity:int):
        return {
            'source': source,
            'target': target,
            'capacity': capacity
    ...
    def create_topologies(self):
        ...
        for i, topo in enumerate(topos):
            ...
            for j in range(self.satellite_num):
                # Uplink
                links.append(self._link(self._user_node(j), j, uplink_cap))
                # Downlink
                links.append(self._link(j, self._user_node(j), downlink_cap))
            ...
            for l in topo[2]:
                ...
                links.append(self._link(s, t, isl_cap))
                
            topo_obj = copy.deepcopy(template)
            topo_obj['links'] = links
            with open(AssetManager.graph_path(self.output_path, i, True), 'w') as f:
                f.write(json.dumps(topo_obj))
